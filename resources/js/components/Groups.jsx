        ...
        <div className="bg-royal-green">
            <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 py-24">
                ...
                <div className="text-center mb-16">
                    <div className="mb-6">
                        <span className="text-sm font-medium text-gold tracking-wider uppercase">GROUPES DE TRAVAIL</span>
                    </div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-16">
                    {Object.entries(GROUPS_CONFIG).map(([key, group]) => (
                        <div key={key} className="bg-white/10 backdrop-blur-sm p-6 rounded-lg border border-gold/30 hover:bg-white/15 transition-all duration-300">
                            <h3 className="text-lg font-semibold text-gold mb-3">{group.title}</h3>
                            ...
                            <div className="space-y-2">
                                {group.objectives.map((objective, index) => (
                                    <div key={index} className="flex items-center gap-2">
                                        ...
                                        <span className="text-white/80 text-xs">{objective}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
                ...
                <div className="grid lg:grid-cols-2 gap-16 items-start">
                    ...
                    <div>
                        <div className="bg-white/10 backdrop-blur-sm p-8 rounded-lg border border-gold/30">
                            <h3 className="text-2xl font-semibold text-gold mb-6">Rejoindre un Groupe</h3>
                    ...
                    <div>
                        <div className="bg-white/10 backdrop-blur-sm p-8 rounded-lg border border-gold/30">
                            <h3 className="text-2xl font-semibold text-gold mb-6">Groupe Sélectionné</h3>
                            
                            <div className="space-y-6">
                                <div>
                                    <h4 className="text-xl font-semibold text-gold mb-3">{selectedGroup.title}</h4>
                                    <p className="text-white leading-relaxed">{selectedGroup.description}</p>
                                </div>

                                <div>
                                    <h5 className="text-lg font-semibold text-gold mb-4">Objectifs du groupe</h5>
                                        ...
                                        {selectedGroup.objectives.map((objective, index) => (
                                            <div key={index} className="flex items-center gap-3">
                                                <div className="w-6 h-6 bg-gold rounded-full flex items-center justify-center flex-shrink-0">
                                                    <span className="text-royal-green text-xs font-bold">{index + 1}</span>
                                                </div>
                                ...
                                {done && (
                                    <div className="pt-6 border-t border-gold/30">
                                        <div className="text-center">
                                            <h5 className="text-lg font-semibold text-gold mb-4">Rejoindre la communauté</h5>
                                            <a 
                                                href={link} 
                                                target="_blank" 
                                                rel="noreferrer"
                                                className="inline-flex items-center gap-3 bg-gold text-royal-green px-8 py-4 rounded-lg font-semibold hover:bg-gold/90 transition-colors duration-300"
                                            >
